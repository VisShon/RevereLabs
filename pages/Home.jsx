...
function Home({Projects}) {
  ...
  return (
    <div className={styles.container}>
        ...
        <div className="relative z-10 flex w-[100%] items-center justify-between overflow-visible">
        ...
        <div className="flex flex-wrap relative z-0">
            {Projects.filter(item=>!filters.includes(item.category)).filter(item=>item.title===searchParam||searchParam=='').map((project, i) =>(<ProjectCard isGigActive={!project.completed}  jobTitle={project.title} amount={project.bounty} descp={project.description} key={i}/>  ))}
        </div>
    </div>
  )
}
