    ...
    return (
      <div className={styles.container}>
          ...
          <div className="relative z-20 flex w-[100%] items-center  overflow-visible">
              ...
              <h2 className="font-mada font-[800] text-main text-[5rem] mx-[30%]">Le Daos</h2>
          </div>
  
          <div className="relative z-10 flex w-[95%] items-center justify-end overflow-visible">
              <Button Content={'Apply Dao'}  Link={'https://linktr.ee/reverelabs'}/>
          </div>
  
  
          <div className="flex flex-wrap w-[100%] justify-items-center  relative z-0">
              {Daos.filter(item=>item.title==searchParam||searchParam=='')
              .map((Dao, i) =>(<DaoCard daoImage={'/dao.png'} gigsProvided={Dao.gigsProvided} gigsDone={Dao.gigsDone} memberCount={Dao.memberCount} descp={Dao.description} link={Dao.link} key={i}/>  ))}
          </div>
      </div>
    )
  }
