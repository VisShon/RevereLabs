...
function LinkCarousel() {
  ...
  return (
    <>
      ...
      {
        addLink?(<div className="relative w-[12rem] h-[2.5rem] z-[10] mt-5">
                    <input value={newLink} onChange={(e)=>setNewLink(e.target.value)} placeholder="Search" type="search" className="p-1 relative z-[5] border-2 rounded-md cursor-text drop-shadow-[5px_5px_0px_rgba(0,0,0,1)]   bg-[#ffffff]"/>
                    <div className="absolute pt-10 top-1 z-[0] w-[12rem] min-h-[5rem] flex items-center justify-around border-2 rounded-b-md font-mada bg-accent">
                      {console.log(newLink)}
                      {linkData.map((item, i)=>(
                ...
                (<div onClick={()=>setAddLink(preVal=>!preVal)} className="w-[12rem] flex justify-center mt-5">
