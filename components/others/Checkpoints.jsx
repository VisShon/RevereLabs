...
function Checkpoints() {
  ...
  return (
    <div className="bg-accent  w-[100%] h-[40%] flex justify-between">
        ...
        <div className="w-[50%] h-[100%] flex flex-col justify-around  relative" onClick={onclickHandler}>
            ...
            {isOpen&&<div className="absolute pt-5 top-[5rem] z-[0] w-[100%] min-h-[5rem] flex flex-col  justify-around border-2 rounded-md font-mada bg-[white]">
                {allTags.map((tags, i) =>(<div onClick={onTagClickHandler(tags)} className="rounded-md px-2 m-1 cursor-pointer" key={i} style={{backgroundColor:tags.colour,color:"black"}}>{tags.title}</div>))}
            </div>}


            <div>
                <p>Platform Fees</p>
                <input className="border-2 rounded-md" placeholder="$10000"/>
            </div>
        </div>
